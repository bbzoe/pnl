...
<div class="container" class:blurred={isLocked} class:tma-mode={$isTMA}>
  ...
  {:else}
    <!-- In TMA mode, just show settings button - Telegram provides the header -->
...
<style>
  /* TMA mode adjustments - account for Telegram native header */
  .container.tma-mode {
    padding-top: calc(env(safe-area-inset-top, 0px) + 3.5rem);
    padding-bottom: calc(env(safe-area-inset-bottom, 0px) + 1rem);
  }

  /* TMA settings row - just a settings button aligned right */
  .tma-settings-row {
    display: flex;
